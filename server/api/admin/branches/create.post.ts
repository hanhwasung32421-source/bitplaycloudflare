...
export default defineEventHandler(async (event) => {
  ...
  const u = await db.prepare('SELECT id FROM users WHERE username = ?').get(body.username) as { id: number }
  await db.prepare('INSERT OR IGNORE INTO balances (user_id, usdt) VALUES (?, ?)').run(u.id, 0)
